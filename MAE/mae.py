...
class MaskedAutoencoderLIT(LightningModule):
    ...
    def generate_grid_of_samples(self, true_images: torch.Tensor, pred_patch: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """
        Generate a grid of samples for visualization with grayscale images.

        Args:
            true_images: Tensor containing the true grayscale images (B, 1, H, W)
            pred_patch: Tensor containing the predicted patches
            mask: Tensor containing the mask

        Returns:
            Tensor representing the grid of samples (H, W, 1)
        """
        ...
        # Get the grayscale images for each sample
        pred_images = res[sample_indices]
        true_image_grid = true_images[sample_indices]

        # Ensure the images are in the correct format (B, C, H, W)
        if len(pred_images.shape) == 3:
            pred_images = pred_images.unsqueeze(1)
        if len(true_image_grid.shape) == 3:
            true_image_grid = true_image_grid.unsqueeze(1)

        # Concatenate true and predicted images
        ...
        # For grayscale, the output shape will be (H, W, 1)
        full_grid = full_grid.permute(1, 2, 0).float().cpu().numpy()
        
        # If the output has 3 channels (due to make_grid), convert to single channel
        if full_grid.shape[-1] == 3:
            full_grid = full_grid.mean(axis=-1, keepdims=True)
